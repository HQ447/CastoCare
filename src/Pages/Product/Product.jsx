...
function Product() {
  const { id } = useParams();
  const resturents = useSelector((state) => state.store.restArr);
  const cart = useSelector((state) => state.store.cart);
  // const foodItems = useSelector((state) => state.store.foodItems);
  const dispatch = useDispatch();
  // const resID = useSelector((state) => state.store.resID);

  const selectedResturent = resturents.find((resturent) => resturent.id == id);
    ...
    <div className="pt-16 px-20">
        ...
        <div className=" flex ">
          ...
          <div className=" w-2/3">
            <h2 className=" text-3xl font-bold">
              {selectedResturent.ResturentName}
            </h2>
            <p className=" my-3">{selectedResturent.description}</p>
      ...
      <div className=" w-full  rounded-xl flex flex-col px-2 my-4 py-5">
        ...
        <div className="flex gap-2">
          {selectedResturent.foodItems.map((obj) => (
            <div
              key={obj.index}
